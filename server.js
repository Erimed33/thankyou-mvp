...
// API endpoint for generating thank you notes
app.post('/generate', async (req, res) => {
  try {
    const { name, product, messageType = 'thankyou' } = req.body;
    ...
    if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'your_openai_api_key_here') {
      ...
      return res.status(200).json({ note: randomMessage });
    }
    ...
    res.status(200).json({ note: message || `Dear ${name}, thank you so much for purchasing ${product}! We truly appreciate your business and hope you love your new purchase.` });
  } catch (err) {
    console.error(err);
